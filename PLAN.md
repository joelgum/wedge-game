...
| Low (trough), **held 1s** | KNEE DROP + hand drag | LAY-BACK, arm spread | Must be **set in the lean for a full second** before it counts at all — let go sooner and the whole thing is forfeit. Locking it in pays a flat **+400** on top of the 240/s the lean earns; hold longer for more. Steering drops to half throughout. |
...
**The held stance is a commitment (2026-07-31, Joel's call).** `STANCE_MIN` = 1.0s. Points accumulate but are **banked only when the lean locks**, so releasing early — or slipping out of the band mid-hold — forfeits everything you'd earned in it. A hold meter fills above the rider through that first second and flashes SET when it takes, because a rule that punishing has to be visible. Locking pays `STANCE_LOCK` (+400) on top, since holding the lean is the risky part: a full second is a second of half-rate steering while the channel keeps wandering under you. Applies to both riders' held stances. Letting go early costs the trick but does **not** break an existing chain.
